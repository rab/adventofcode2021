...
# experimental antenna that should be able to track the keys if you can boost its signal strength
# high enough; there's a little meter that indicates the antenna's signal strength by displaying
# 0-50 stars.
#
...
input.each_line(chomp: true) do |line|
  ...
  if (curr = line.to_i) > prev
    increases += 1
  end
  prev = curr
end

puts "Part 1: increases: %d"%[increases]
...
prev = nil
increases = 0
input.each_line(chomp: true).map(&:to_i).each_cons(3) do |depths|
  puts depths.inspect if debugging
  prev ||= depths.sum

  if (curr = depths.sum) > prev
    increases += 1
  end
  prev = curr
end

puts "Part 2: increases: %d"%[increases]
